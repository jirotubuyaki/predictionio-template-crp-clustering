...
object CRPClustering extends java.io.Serializable{
	var data_length: Int = 0
	var dim: Int = 0
	def crp_gibbs(data_raw: PreparedData, mu: List[Double], sigma_table: Double, alpha: Double, ro_0: Double, burn_in: Int, iteration: Int): Array[Array[Double]] = {
		data_length = (data_raw.points.count).toInt
		dim = (data_raw.points.first.points_seq.length).toInt
		var data_points: Array[Point] = data_raw.points.collect()
		...
		for(i <-0 to data_length - 1){
			for(j <- 0 to dim - 1){
				data(i, j) = data_points(i).points_seq(j)
			}
			println("data: " + data(i,::))
		}
